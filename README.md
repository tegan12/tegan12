<div align="center">

# Hi, I'm Tegan 👋

### IT Support & Systems Administration · Ireland 🇮🇪

![Open to Work](https://img.shields.io/badge/Open%20to%20Work-IT%20Support%20%2F%20SysAdmin-2ea44f)
![Location](https://img.shields.io/badge/Based%20in-Ireland-0b7285)
![Right to Work](https://img.shields.io/badge/Right%20to%20Work-Stamp%201G-0078D4)

</div>

Final-year IT student specialising in **Systems Infrastructure & Network Security**.
I don't just fix problems — I **automate** them. Right to work: **Stamp 1G** (full-time, no employer permit needed).

- 🎯 Targeting **IT Support / Service Desk / Systems Administration** roles — SME · MSP · in-house IT
- 🧰 Day-to-day: **Active Directory · Microsoft 365 · Azure/Entra · Windows Server · Linux · PowerShell**
- 📜 Studying toward **Microsoft AZ-900** & **MS-900**
- 🌱 Growth path: support → automation → cloud & security
- 💼 [linkedin.com/in/tegan-wilton](https://linkedin.com/in/tegan-wilton)

---

### 🛠️ Tech I work with

**Systems & Directory**

![Windows Server](https://img.shields.io/badge/Windows_Server-0078D6?logo=data:image/svg+xml;base64,PHN2ZyByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%2BV2luZG93czwvdGl0bGU%2BPHBhdGggZD0iTTAsMEgxMS4zNzdWMTEuMzcySDBaTTEyLjYyMywwSDI0VjExLjM3MkgxMi42MjNaTTAsMTIuNjIzSDExLjM3N1YyNEgwWm0xMi42MjMsMEgyNFYyNEgxMi42MjMiLz48L3N2Zz4%3D&logoColor=white)
![Active Directory](https://img.shields.io/badge/Active_Directory-1f6feb?logoColor=white)
![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?logo=data:image/svg+xml;base64,PHN2ZyByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%2BTWljcm9zb2Z0IE9mZmljZTwvdGl0bGU%%2BPC9zdmc%2B&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)
![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?logo=ubuntu&logoColor=white)

**Cloud & Networking**

![Azure](https://img.shields.io/badge/Azure-0078D4?logo=data:image/svg+xml;base64,PHN2ZyByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%%3D&logoColor=white)
![Microsoft Entra](https://img.shields.io/badge/Microsoft_Entra_ID-0067B8?logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?logo=cisco&logoColor=white)
![Networking](https://img.shields.io/badge/TCP%2FIP%20·%20DNS%20·%20DHCP-3b5bdb?logoColor=white)

**Automation & Scripting**

![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?logo=data:image/svg+xml;base64,PHN2ZyByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%2BUG93ZXJTaGVsbDwvdGl0bGU%%3D%3D&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?logo=gnubash&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![MariaDB](https://img.shields.io/badge/MariaDB-003545?logo=mariadb&logoColor=white)

**Tooling & Practice**

![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![VirtualBox](https://img.shields.io/badge/VirtualBox-183A61?logo=virtualbox&logoColor=white)
![osTicket](https://img.shields.io/badge/osTicket-1B4F72?logoColor=white)
![ITIL](https://img.shields.io/badge/ITIL-Service_Management-00B2A9?logoColor=white)

---

### 📌 Featured projects
Each repo has a full write-up, working automation, and **real screenshots of it running**.

- 🖥️ **[ad-home-lab](https://github.com/tegan12/ad-home-lab)** — a Windows **domain built from scratch**:
  Active Directory, DNS, DHCP & Group Policy, with **PowerShell** bulk user-onboarding. Includes a
  domain-joined Windows 11 client with policy verified via `gpresult`.
- 🎫 **[helpdesk-ticketing](https://github.com/tegan12/helpdesk-ticketing)** — a self-hosted **osTicket**
  service desk on **Ubuntu/LAMP**, run through a real **ITIL** incident workflow (impact × urgency
  priority matrix + SLAs), with tickets worked end-to-end.
- ⚙️ **[powershell-toolkit](https://github.com/tegan12/powershell-toolkit)** — practical **IT-support
  automation**: system health reports, AD password-expiry alerts, software inventory, safe cleanup and
  endpoint checks.

🧪 Also built a live **AI exam-grading web app** on **Cloudflare Workers + Claude API** →
**[en1090-study-site.pages.dev](https://en1090-study-site.pages.dev)**

---

<div align="center">

### 📊 GitHub

![Tegan's GitHub stats](https://github-readme-stats.vercel.app/api?username=tegan12&show_icons=true&hide_border=true&count_private=true)

![Top languages](https://github-readme-stats.vercel.app/api/top-langs/?username=tegan12&layout=compact&hide_border=true&langs_count=8)

📍 Open to **hybrid / remote** roles across Ireland · **[Connect on LinkedIn »](https://linkedin.com/in/tegan-wilton)**

</div>
